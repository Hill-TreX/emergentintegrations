...
exports.OpenAIVideoGeneration = void 0;
/**
...
class OpenAIVideoGeneration {
     ...
     */
    async textToVideo(prompt, model = "sora-2", size = "1280x720", duration = 4, maxWaitTime = 600, imagePath, mimeType = "image/jpeg") {
        // Validate parameters
        if (!OpenAIVideoGeneration.MODELS.includes(model)) {
            throw new Error(`Invalid model: ${model}. Must be one of ${OpenAIVideoGeneration.MODELS}`);
        }
        if (!(size in OpenAIVideoGeneration.SIZES)) {
            throw new Error(`Invalid size: ${size}. Must be one of ${Object.keys(OpenAIVideoGeneration.SIZES)}`);
        }
        if (!OpenAIVideoGeneration.DURATIONS.includes(duration)) {
            throw new Error(`Invalid duration: ${duration}. Must be one of ${OpenAIVideoGeneration.DURATIONS}`);
        }
        // Initiate video generation
        const operationId = await this._generateVideo(prompt, model, size, duration, imagePath, mimeType);
        if (!operationId)
            return null;
        // Wait for completion
    ...
    async _generateVideo(prompt, model, size, duration, imagePath, mimeType = "image/jpeg") {
        ...
        try {
            ...
            const operationId = data.id || data.operation_id || data.generation_id || null;
            if (operationId) {
    ...
    async _waitForCompletion(operationId, maxWaitTime = 600) {
        const operationUrl = `${this.baseUrl}/videos/${operationId}`;
        const startTime = Date.now();
        let pollInterval = 10000; // Start with 10 seconds (in ms)
        console.log(`Waiting for video generation to complete (ID: ${operationId})...`);
        while (Date.now() - startTime < maxWaitTime * 1000) {
            try {
                ...
                if (!status)
                    status = (data.job_status || "").toLowerCase();
                // Check if completed
                if (["completed", "complete", "succeeded", "success", "done"].includes(status)) {
                    ...
                    if (!videoUri) {
                        ...
                        console.log(`No URL in response, constructed download URL: ${videoUri}`);
                    }
                    if (videoUri) {
                    ...
                    console.log(`Still processing... (${elapsed}s elapsed, status: ${status}, progress: ${progress}%)`);
                }
                else {
                    ...
                    if (data.video_url ||
                        ...
                        data.download_url ||
                        data.output_url) {
                        const uri = data.video_url || data.url || data.download_url || data.output_url;
                        if (uri)
                            return uri;
                    }
                }
        ...
        console.error(`Timeout: Video generation did not complete within ${maxWaitTime} seconds`);
        return null;
    }
    async _downloadVideoBytes(videoUri) {
        try {
            ...
            if (!contentType.startsWith("video/") &&
                ...
                console.log("Content type doesn't look like video, trying fallback approach");
            ...
            if (videoData.length < 1000) {
                console.error(`Error: Video data too small (${videoData.length} bytes)`);
                return null;
            }
            return videoData;
        }
     ...
     */
    saveVideo(videoBytes, outputPath) {
...
exports.OpenAIVideoGeneration = OpenAIVideoGeneration;
/** Supported models */
OpenAIVideoGeneration.MODELS = ["sora-2", "sora-2-pro"];
...
OpenAIVideoGeneration.DURATIONS = [4, 8, 12];
